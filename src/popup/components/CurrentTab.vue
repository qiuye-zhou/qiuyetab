...
<template>
  <div
    class="px-4 py-3"
    v-show="tabs.length > 0 && !showSettings && !showEditSites"
  >
    ...
    <div class="space-y-2 max-h-32 overflow-y-auto">
      ...
      >
        <div
          class="w-4 h-4 mr-2 flex-shrink-0 flex items-center justify-center"
        >
          <img
            v-if="tab.favIconUrl"
            :src="tab.favIconUrl"
        ...
        <span
          class="text-xs text-gray-700 dark:text-gray-200 truncate flex-1"
          >{{ tab.title }}</span
        >
        <button
