...
<template>
  <div>
    <h3 class="text-lg font-medium text-gray-800 mb-4">搜索引擎设置</h3>
    <div class="space-y-3">
      <label v-for="engine in searchEngines" :key="engine.value"
        class="flex items-center p-3 rounded-lg border border-gray-200 hover:bg-gray-50 cursor-pointer transition-colors duration-200"
        :class="{ 'border-blue-300 bg-blue-50': selectedEngine === engine.value }">
        <input type="radio" :value="engine.value" v-model="selectedEngine" class="sr-only" />
        <div class="flex items-center space-x-3">
          <div class="w-4 h-4 rounded-full border-2 flex items-center justify-center"
            :class="selectedEngine === engine.value ? 'border-blue-500' : 'border-gray-300'">
